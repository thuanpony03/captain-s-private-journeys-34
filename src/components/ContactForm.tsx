...
const ContactForm = () => {
  ...
  const priorities = [{
    value: "health",
    label: "Sức khỏe",
    desc: "Lịch nhẹ nhàng, thoải mái"
  }, {
    value: "experience",
    label: "Trải nghiệm",
    desc: "Văn hóa, ẩm thực độc đáo"
  }, {
    value: "luxury",
    label: "Sang trọng",
    desc: "Check-in đẳng cấp"
  }];
  return <section ref={sectionRef} id="contact-form" className="py-12 md:py-20 lg:py-28 relative overflow-hidden bg-gradient-to-b from-[#faf9f7] via-white to-[#faf9f7]">
      {/* Background Pattern */}
                ...
                <form onSubmit={handleSubmit} className="p-5 md:p-8 space-y-6">
                  ...
                  <div>
                    ...
                    destination: value
                      ...
                      {destinations.map(dest => <div key={dest.value} className="relative">
                          <RadioGroupItem value={dest.value} id={dest.value} className="sr-only peer" />
                          <Label htmlFor={dest.value} className={`
                              block cursor-pointer rounded-xl overflow-hidden border-2 transition-all duration-300
                              ${formData.destination === dest.value ? 'border-primary shadow-lg shadow-primary/20 scale-[1.02]' : 'border-transparent hover:border-primary/20'}
                            `}>
                            {/* Image */}
                            <div className="relative h-20 md:h-24 overflow-hidden">
                              ...
                              {/* Selected indicator */}
                              {formData.destination === dest.value && <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-primary flex items-center justify-center">
                  ...
                  <div>
                    ...
                    groupSize: value
                      ...
                      {groupSizes.map(size => <div key={size.value} className="relative">
                          <RadioGroupItem value={size.value} id={`size-${size.value}`} className="sr-only peer" />
                          <Label htmlFor={`size-${size.value}`} className={`
                          ...
                          {size.hot && <span className="absolute -top-2 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-secondary text-primary text-[10px] font-bold rounded-full whitespace-nowrap">
                              PHỔ BIẾN
                            </span>}
                        </div>)}
                    </RadioGroup>
                  ...
                  <div>
                    ...
                    <Input id="contact" type="tel" placeholder="VD: 0901 234 567" value={formData.contact} onChange={e => setFormData({
                    ...formData,
